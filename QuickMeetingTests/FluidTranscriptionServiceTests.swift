...
struct FluidTranscriptionServiceTests {
    ...
    @Test
    func realtimeCoordinatorStoresUnavailableMessageWhenAppendFails() async throws {
        let harness = try RealtimeTranscriptionHarness()
        let meeting = try harness.createMeeting()
        let transcriber = FailingRealtimeTranscriber(error: RealtimeTranscriptionTestError.backendDown)
        let coordinator = RealtimeTranscriptionCoordinator(
            meetingStore: harness.meetingStore,
            dateProvider: { Date(timeIntervalSince1970: 1_234_568_000) },
            transcriberFactory: { _, _ in transcriber }
        )
        ...
        coordinator.append(RealtimeAudioBuffer(samplesByChannel: [[0.1, 0.2]], sampleRate: 48_000))
        try await harness.waitForRealtimeTranscript(
            meetingID: meeting.id,
            expectedText: "Realtime transcription unavailable: backend down"
        )

        let reloaded = try harness.reloadMeeting(id: meeting.id)
        #expect(reloaded.realtimeTranscriptText == "Realtime transcription unavailable: backend down")
    }

    @Test
    func realtimeCoordinatorExplainsFluidAudioHuggingFaceRateLimit() async throws {
        let harness = try RealtimeTranscriptionHarness()
        let meeting = try harness.createMeeting()
        let transcriber = FailingRealtimeTranscriber(
            startError: DownloadUtils.HuggingFaceDownloadError.rateLimited(
                statusCode: 429,
        ...
        try await harness.waitForRealtimeTranscript(
            meetingID: meeting.id,
            expectedText: "Realtime transcription unavailable: Fluid Audio models are not cached yet and Hugging Face rate-limited the download. Set HF_TOKEN or HUGGING_FACE_HUB_TOKEN, configure REGISTRY_URL, or retry later; after the models are cached, realtime transcription runs locally."
        )

        let reloaded = try harness.reloadMeeting(id: meeting.id)
        #expect(reloaded.realtimeTranscriptText?.contains("Rate limited while listing files") == false)
    }

    @Test
    func realtimeTranscriberDefaultsToFluidAudioForRussian() {
        let transcriber = RealtimeTranscriptionCoordinator.makeTranscriber(
        ...
        #expect(transcriber is FluidRealtimeTranscriber)
    }

    @Test
    func realtimeTranscriberUsesCustomBackendWhenConfigured() {
        let transcriber = RealtimeTranscriptionCoordinator.makeTranscriber(
            options: TranscriptionPipelineOptions(
                languageCode: "ru",
                isRealtimeTranscriptionEnabled: true,
        ...
        #expect(transcriber is CustomOpenAICompatibleRealtimeTranscriber)
    }
}
...
@MainActor
private struct RealtimeTranscriptionHarness {
    let container: ModelContainer
    let meetingStore: MeetingStore
    let fileManager: FileManager
    let meetingFileStore: MeetingFileStore
    ...
    func createMeeting() throws -> Meeting {
        let startedAt = Date(timeIntervalSince1970: 1_234_567_890)
        let artifacts = try meetingFileStore.createArtifacts(for: UUID(), startedAt: startedAt)
        return try meetingStore.createMeeting(
            id: UUID(uuidString: artifacts.meetingFolderURL.lastPathComponent) ?? UUID(),
            title: "Design Review",
            startedAt: startedAt,
            folderURL: artifacts.meetingFolderURL,
    ...
    func waitForRealtimeTranscript(meetingID: UUID, expectedText: String) async throws {
        for _ in 0..<50 {
            if try reloadMeeting(id: meetingID).realtimeTranscriptText == expectedText {
                return
            }
            try await Task.sleep(nanoseconds: 20_000_000)
        }
        Issue.record("Timed out waiting for realtime transcript")
    }
}

private enum RealtimeTranscriptionTestError: LocalizedError {
    case backendDown
...
private actor FailingRealtimeTranscriber: RealtimeTranscribing {
    let startError: Error?
    let error: Error

    init(
        startError: Error? = nil,
        error: Error = RealtimeTranscriptionTestError.backendDown
    ) {
    ...
    func append(_: RealtimeAudioBuffer) async throws {
        throw error
    }
